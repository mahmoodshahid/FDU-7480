...
window.shareSummaryAsImage = function() {
    ...
    container.innerHTML = `
        <div class="png-report-header">
            ...
            <p style="margin: 2px 0 0; font-size: 14px; font-weight:800; color:#fbbf24; text-transform:uppercase;">${userName}</p>
        ...
        <div class="png-report-body">
            ...
            <div style="text-align: center; margin-top: 20px; font-size: 11px; color: #64748b;">
                Report generated by ${userName} on ${new Date().toLocaleDateString()}
            </div>
        </div>
    `;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const shareCurrentWhatsAppBtn = document.getElementById('shareCurrentWhatsApp');
    ...
    if (shareCurrentWhatsAppBtn) {
        shareCurrentWhatsAppBtn.addEventListener('click', () => {
            const date = document.getElementById('dateGoing').value;
            const from = document.getElementById('rawangiGoing').value;
            const to = document.getElementById('stopGoing').value;
            const fare = calculateFare();
            const expenses = calculateExpenses();
            const bachat = fare - expenses;
            const userName = userNameInput.value || "Driver";

            const text = `*FDU 7480 Trip Report*\n` +
                         `*Driver:* ${userName}\n` +
                         `*Date:* ${formatDate(date)}\n` +
                         `*Route:* ${from} to ${to}\n` +
                         `--------------------------\n` +
                         `*Total Fare:* Rs. ${fare.toLocaleString()}\n` +
                         `*Total Expenses:* Rs. ${expenses.toLocaleString()}\n` +
                         `--------------------------\n` +
                         `*${bachat >= 0 ? 'Bachat' : 'Nuqsan'}:* Rs. ${Math.abs(bachat).toLocaleString()}\n\n` +
            ...
            row.innerHTML = `
                ...
                <td class="action-btns-cell">
                    <div style="display:flex; flex-direction:column; gap:4px;">
                        <button class="btn-action btn-share-wa" data-id="${record.id}" style="padding:4px 8px; font-size:10px; background:#25d366; color:white;">WA</button>
                        ...
                        <button class="btn-action btn-share" data-id="${record.id}" style="padding:4px 8px; font-size:10px; background:#3b82f6; color:white;">PDF</button>
    ...
    function shareRecordWhatsApp(id) {
        ...
        const text = `*FDU 7480 Trip Report*\n` +
                     `*Driver:* ${userName}\n` +
                     `*Date:* ${formatDate(record.dateGoing)}\n` +
                     `*Route:* ${record.rawangiGoing} → ${record.stopGoing}\n` +
                     `--------------------------\n` +
                     `*Total Fare:* Rs. ${record.totalKiraya.toLocaleString()}\n` +
                     `*Total Expenses:* Rs. ${record.totalExpenses.toLocaleString()}\n` +
                     `--------------------------\n` +
                     `*${record.balance >= 0 ? 'Bachat' : 'Nuqsan'}:* Rs. ${Math.abs(record.balance).toLocaleString()}\n\n` +
                     `Generated by FDU 7480 App`;
    ...
    function shareRecordAsPNG(id) {
        ...
        const bachatStatus = record.balance >= 0 ? 'Profit (Bachat)' : 'Loss (Nuqsan)';
        ...
        container.innerHTML = `
            ...
            <div class="png-report-body">
                ...
                <div class="png-row">
                    <span class="png-label">Total Expenses (اخراجات):</span>
                    <span class="png-value" style="color:#dc2626;">Rs. ${record.totalExpenses.toLocaleString()}</span>
                </div>
                <div class="png-row" style="margin-top: 10px; border-top: 3px double #333; padding-top: 10px;">
                ...
                <div style="text-align: center; margin-top: 20px; font-size: 12px; color: #64748b; border-top: 1px solid #eee; padding-top: 10px;">
                    Generated by ${userName} for FDU 7480
                </div>
            </div>
        `;
