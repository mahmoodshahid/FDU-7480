...
window.shareSummaryAsPDF = function() {
    ...
    if (records.length === 0) {
        alert("No records | کوئی ریکارڈ نہیں");
        return;
    }

    const totals = {
        diesel: 0, toll: 0, khana: 0, driver: 0, digar: 0,
    ...
    element.innerHTML = `
        <div style="text-align: center; border-bottom: 3px solid #1e40af; padding-bottom: 20px; margin-bottom: 30px;">
            <h1 style="font-size: 38px; margin: 0; color: #1e40af;">FDU 7480</h1>
        ...
        <div style="border: 2px solid #e2e8f0; border-radius: 12px; padding: 25px;">
            ...
            <div style="text-align: center; margin-top: 50px; font-size: 14px; color: #64748b; border-top: 1px solid #e2e8f0; padding-top: 20px;">
                Generated for FDU 7480 on ${new Date().toLocaleDateString()}
            </div>
        </div>
    `;
    ...
    window.html2pdf().from(element).set(opt).toPdf().get('pdf').then(function(pdf) {
        // Direct save is most reliable on mobile browsers
        pdf.save('FDU7480_Summary.pdf');
    }).catch(err => {
        alert("Could not create PDF on this device.");
    }).finally(() => {
...
function forceDownload(blob, fileName) {
    // For mobile, pdf.save() or direct blob open is more reliable
...
window.downloadReceiptPDF = function() {
    ...
    window.html2pdf().from(element).set(opt).toPdf().get('pdf').then(function(pdf) {
        pdf.save(`FDU7480_Trip_${Date.now()}.pdf`);
    }).catch(err => {
        alert("Could not create PDF on this device.");
    }).finally(() => {
        btn.innerHTML = originalText;
